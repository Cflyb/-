          ...
          <form id="billForm" name="billForm" method="post" action="${pageContext.request.contextPath }/bill/modifysave.html">
                ...
                <div>
					...
					<select name="providerId" id="providerId">
                        <option value="1" <c:if test="${bill.providerId eq 1}">selected</c:if>>山东豪克华光联合发展有限公司</option>
                        <option value="2" <c:if test="${bill.providerId eq 2}">selected</c:if>>北京国粮食用油有限公司</option>
                        <option value="3" <c:if test="${bill.providerId eq 3}">selected</c:if>>北京三木堂商贸有限公司	</option>
		        	</select>
					<font color="red"></font>
                </div>
                <div>
