...
function deleteProvider(obj){
    $.ajax({
        ...
        success:function(data){
            if(data.roleResult == "true"){//删除成功：移除删除行
                cancleBtn();
                obj.parents("tr").remove();
            }else if(data.roleResult == "false"){//删除失败
                //alert("对不起，删除供应商【"+obj.attr("proname")+"】失败");
                changeDLGContent("对不起，删除角色【"+obj.attr("proname")+"】失败");
            }else if(data.roleResult == "notexist"){
...
$(function(){
    ...
    $(".modifyRole").on("click",function(){
        var obj = $(this);
        window.location.href=path+"/role/tomodify.html?roleid="+ obj.attr("roleid");
    });
    ...
    $(".roledelete").on("click",function(){
        providerObj = $(this);
        changeDLGContent("你确定要删除供应商【"+providerObj.attr("rolename")+"】吗？");
        openYesOrNoDLG();
    });
